...
#if (defined(__cpp_exceptions) && __cpp_exceptions) || (defined(__EXCEPTIONS) &&
 __EXCEPTIONS)
...
--
  ...
  struct Rect {
    ...
    friend std::ostream& operator<<(std::ostream& os, const Rect<T>& rect) {
      os << "(" << rect.left << "," << rect.top << "," << rect.right << "," << r
ect.bottom << ") ";
      return os;
    }
  };
    ...
    double max_dist_sqrd, bool is_closed_path)
  {
    ...
    {
      result.emplace_back(std::move(StripNearEqual(*paths_citer, max_dist_sqrd,
is_closed_path)));
    }
    return result;
  }
  ...
  {
    //https://stackoverflow.com/questions/1041620/whats-the-most-efficient-way-t
o-erase-duplicates-and-sort-a-vector#:~:text=Let%27s%20compare%20three%20approac
hes%3A
  ...
  inline void CheckPrecisionRange(int& precision, int& error_code)
  {
    ...
    DoError(precision_error_i);      // does nothing when exceptions are disable
d
    precision = precision > 0 ? CLIPPER2_MAX_DEC_PRECISION : -CLIPPER2_MAX_DEC_P
RECISION;
  }
  ...
  template <typename T>
  inline int CrossProductSign(const Point<T>& pt1, const Point<T>& pt2, const Po
int<T>& pt3)
  {
  ...
  template <typename T>
  inline double CrossProduct(const Point<T>& pt1, const Point<T>& pt2, const Poi
nt<T>& pt3) {
    return (static_cast<double>(pt2.x - pt1.x) * static_cast<double>(pt3.y -
      pt2.y) - static_cast<double>(pt2.y - pt1.y) * static_cast<double>(pt3.x -
pt2.x));
  }
  ...
  {
    return static_cast<double>(vec1.y * vec2.x) - static_cast<double>(vec2.y * v
ec1.x);
  }

  template <typename T>
  inline double DotProduct(const Point<T>& pt1, const Point<T>& pt2, const Point
<T>& pt3) {
    return (static_cast<double>(pt2.x - pt1.x) * static_cast<double>(pt3.x - pt2
.x) +
  ...
  {
    return static_cast<double>(vec1.x * vec2.x) + static_cast<double>(vec1.y * v
ec2.y);
  }
    ...
    const Point<T>& ln2a, const Point<T>& ln2b, Point<T>& ip)
  {
    ...
    if constexpr (std::is_integral_v<T>)
    {
      int64_t originx = (CC_MIN(bb0maxx, bb1maxx) + CC_MAX(bb0minx, bb1minx)) >>
 1;
      int64_t originy = (CC_MIN(bb0maxy, bb1maxy) + CC_MAX(bb0miny, bb1miny)) >>
 1;
    ...
    {
      double originx = (CC_MIN(bb0maxx, bb1maxx) + CC_MAX(bb0minx, bb1minx)) / 2
.0;
      double originy = (CC_MIN(bb0maxy, bb1maxy) + CC_MAX(bb0miny, bb1miny)) / 2
.0;
      double ln0c = (ln1dy * static_cast<double>(ln1a.x - originx)) +
        (ln1dx * static_cast<double>(ln1a.y - originy));
      double ln1c = (ln2dy * static_cast<double>(ln2a.x - originx)) +
  ...
  template <typename T>
  inline PointInPolygonResult PointInPolygon(const Point<T>& pt, const Path<T>&
polygon)
  {
    if (polygon.size() < 3)
    ...
    int val = 0;
    typename Path<T>::const_iterator cbegin = polygon.cbegin(), first = cbegin,
curr, prev;
