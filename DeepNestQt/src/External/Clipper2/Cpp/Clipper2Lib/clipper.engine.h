...
namespace Clipper2Lib {
        ...
        enum class VertexFlags : uint32_t {
                Empty = 0, OpenStart = 1, OpenEnd = 2, LocalMax = 4, LocalMin =
8
        };

        constexpr enum VertexFlags operator &(enum VertexFlags a, enum VertexFla
gs b)
        {
...
gs b)
        ...
        struct OutPt {
                ...
                OutPt(const Point64& pt_, OutRec* outrec_): pt(pt_), outrec(outr
ec_) {
                        next = this;
                        prev = this;
                }
        ...
        //OutRec: contains a path in the clipping solution. Edges in the AEL wil
l
        //have OutRec pointers assigned when they form part of the clipping solu
tion.
        struct OutRec {
                size_t idx = 0;
        ...
        struct Active {
                Point64 bot;
                Point64 top;
                int64_t curr_x = 0;             //current (updated at every new
scanline)
                double dx = 0.0;
                int wind_dx = 1;                        //1 or -1 depending on w
inding direction
                int wind_cnt = 0;
                int wind_cnt2 = 0;              //winding count of the opposite
polytype
                OutRec* outrec = nullptr;
                //AEL: 'active edge list' (Vatti's AET - active edge table)
                //     a linked list of all edges (from left to right) that are
present
                //     (or 'active') within the current scanbeam (a horizontal '
beam' that
                //     sweeps from bottom to top over the paths in the clipping
operation).
                Active* prev_in_ael = nullptr;
                Active* next_in_ael = nullptr;
                //SEL: 'sorted edge list' (Vatti's ST - sorted table)
                //     linked list used when sorting edges into their new positi
ons at the
                //     top of scanbeams, but also (re)used to process horizontal
s.
                Active* prev_in_sel = nullptr;
                Active* next_in_sel = nullptr;
                Active* jump = nullptr;
                Vertex* vertex_top = nullptr;
                LocalMinima* local_min = nullptr;  // the bottom of an edge 'bou
nd' (also Vatti)
        ...
        typedef std::function<void(const Point64& e1bot, const Point64& e1top,
                const Point64& e2bot, const Point64& e2top, Point64& pt)> ZCallb
ack64;

        typedef std::function<void(const PointD& e1bot, const PointD& e1top,
                const PointD& e2bot, const PointD& e2top, PointD& pt)> ZCallback
D;
#endif
...
----
        ...
        public:
                ...
                void Clear();
                void AddPaths(const Paths64& paths, PathType polytype, bool is_o
pen);
        };
...
----
        ...
        private:
                ...
                Active *sel_ = nullptr;
                LocalMinimaList minima_list_;           //pointers in case of me
mory reallocs
        ...
        HorzSegmentList horz_seg_list_;
                ...
                inline void AddLocMin(Vertex &vert, PathType polytype, bool is_o
pen);
                ...
                void AddNewIntersectNode(Active &e1, Active &e2, const int64_t t
op_y);
                ...
                bool ResetHorzDirection(const Active &horz, const Vertex* max_ve
rtex,
        ...
        protected:
                ...
                OutRecList outrec_list_; //pointers in case list memory realloca
ted
                bool ExecuteInternal(ClipType ct, FillRule ft, bool use_polytree
s);
...
pen);
        public:
                ...
                void Clear();
                void AddReuseableData(const ReuseableDataContainer64& reuseable_
data);
#ifdef USINGZ
                int64_t DefaultZ = 0;
#endif
...
-------
        ...
        //PolyTree: is intended as a READ-ONLY data structure for CLOSED paths r
eturned
        //by clipping operations. While this structure is more complex than the
        //alternative Paths structure, it does preserve path 'ownership' - ie th
ose
        //paths that contain (or own) other paths. This will be useful to some u
sers.
...
;
        ...
        private:
                ...
                Path64 polygon_;
        public:
                explicit PolyPath64(PolyPath64* parent = nullptr) : PolyPath(par
ent) {}
                explicit PolyPath64(PolyPath64* parent, const Path64& path) : Po
lyPath(parent) { polygon_ = path; }
                ...
                PolyPath64List::const_iterator begin() const { return childs_.cb
egin(); }
...
(); }
                ...
                {
                        return childs_.emplace_back(std::make_unique<PolyPath64>
(this, path)).get();
                }
                ...
                double Area() const
                {
                        return std::accumulate(childs_.cbegin(), childs_.cend(),
                                ...
                                [](double a, const auto& child) {return a + chil
d->Area(); });
                }
        ...
        private:
                ...
                PathD polygon_;
        public:
                explicit PolyPathD(PolyPathD* parent = nullptr) : PolyPath(paren
t)
                {
                        scale_ = parent ? parent->scale_ : 1.0;
                }

                explicit PolyPathD(PolyPathD* parent, const Path64& path) : Poly
Path(parent)
                {
                        scale_ = parent ? parent->scale_ : 1.0;
                        int error_code = 0;
                        polygon_ = ScalePath<double, int64_t>(path, scale_, erro
r_code);
                }

                explicit PolyPathD(PolyPathD* parent, const PathD& path) : PolyP
ath(parent)
                {
                        scale_ = parent ? parent->scale_ : 1.0;
                        polygon_ = path;
                ...
                PolyPathDList::const_iterator begin() const { return childs_.cbe
gin(); }
                ...
                {
                        return childs_.emplace_back(std::make_unique<PolyPathD>(
this, path)).get();
                }
...
d->Area(); });
        ...
        public:
                ...
                bool Execute(ClipType clip_type, FillRule fill_rule, PolyTree64&
 polytree)
                {
                        Paths64 dummy;
                ...
                bool Execute(ClipType clip_type,
                        FillRule fill_rule, PolyTree64& polytree, Paths64& open_
paths)
                {
        ...
        public:
                ...
                {
                        // de-scale (x & y)
                        // temporarily convert integers to their initial float v
alues
                        // this will slow clipping marginally but will make it m
uch easier
                        // to understand the coordinates passed to the callback
function
...
been assigned
                                ...
                                ClipperBase::zCallback_ =
                                        std::bind(&ClipperD::ZCB, this, std::pla
ceholders::_1,
                                        std::placeholders::_2, std::placeholders
::_3,
                                        std::placeholders::_4, std::placeholders
::_5);
                        else
                ...
                {
                        AddPaths(ScalePaths<int64_t, double>(subjects, scale_, e
rror_code_), PathType::Subject, false);
                }

                void AddOpenSubject(const PathsD& open_subjects)
                {
                        AddPaths(ScalePaths<int64_t, double>(open_subjects, scal
e_, error_code_), PathType::Subject, true);
                }

                void AddClip(const PathsD& clips)
                {
                        AddPaths(ScalePaths<int64_t, double>(clips, scale_, erro
r_code_), PathType::Clip, false);
                }

                bool Execute(ClipType clip_type, FillRule fill_rule, PathsD& clo
sed_paths)
                {
                        PathsD dummy;
                        return Execute(clip_type, fill_rule, closed_paths, dummy
                ...
                bool Execute(ClipType clip_type,
                        FillRule fill_rule, PathsD& closed_paths, PathsD& open_p
aths)
                {
#ifdef USINGZ
                        CheckCallback();
                ...
                bool Execute(ClipType clip_type, FillRule fill_rule, PolyTreeD&
polytree)
                {
                        PathsD dummy;
                        return Execute(clip_type, fill_rule, polytree, dummy);
                ...
                bool Execute(ClipType clip_type,
                        FillRule fill_rule, PolyTreeD& polytree, PathsD& open_pa
ths)
                {
#ifdef USINGZ
                        CheckCallback();
