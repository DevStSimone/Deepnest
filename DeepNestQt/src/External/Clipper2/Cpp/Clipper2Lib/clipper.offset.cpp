...
void GetLowestClosedPathInfo(const Paths64& paths, std::optional<size_t>& idx, b
ool& is_neg_area)
{
        idx.reset();
        ...
        for (size_t i = 0; i < paths.size(); ++i)
        {
                ...
                {
                        if ((pt.y < botPt.y) ||
                                ((pt.y == botPt.y) && (pt.x >= botPt.x))) contin
ue;
                        if (a == MAX_DBL)
                        {
                                a = Area(paths[i]);
                                if (a == 0) break; // invalid closed path, so br
eak from inner loop
                                is_neg_area = a < 0;
                        }
      idx = i;
...
{
        // given that this is an internal function, and given the x and y parame
ters
        // will always be coordinate values (or the difference between coordinat
e values),
...
static inline Point64 GetPerpendic(const Point64& pt, const PointD& norm, double
 delta)
{
#ifdef USINGZ
...
ClipperOffset::Group::Group(const Paths64& _paths, JoinType _join_type, EndType
_end_type):
        paths_in(_paths), join_type(_join_type), end_type(_end_type)
{
        ...
        if (end_type == EndType::Polygon)
        {
                ...
                // the lowermost path must be an outer path, so if its orientati
on is negative,
                // then flag the whole group is 'reversed' (will negate delta et
c.)
...
#ifdef USINGZ
                pt1 = PointD(path[j].x - abs_delta * norms[j].x, path[j].y - abs
_delta * norms[j].y, path[j].z);
                pt2 = PointD(path[j].x + abs_delta * norms[j].x, path[j].y + abs
_delta * norms[j].y, path[j].z);
#else
                pt1 = PointD(path[j].x - abs_delta * norms[j].x, path[j].y - abs
_delta * norms[j].y);
...
_delta * norms[j].y);
        ...
        {
#ifdef USINGZ
                pt1 = PointD(path[j].x + group_delta_ * norms[k].x, path[j].y +
group_delta_ * norms[k].y, path[j].z);
                pt2 = PointD(path[j].x + group_delta_ * norms[j].x, path[j].y +
group_delta_ * norms[j].y, path[j].z);
#else
                pt1 = PointD(path[j].x + group_delta_ * norms[k].x, path[j].y +
group_delta_ * norms[k].y);
                pt2 = PointD(path[j].x + group_delta_ * norms[j].x, path[j].y +
group_delta_ * norms[j].y);
#endif
        }
...
void ClipperOffset::DoSquare(const Path64& path, size_t j, size_t k)
{
        ...
        PointD pt1 = TranslatePoint(ptQ, group_delta_ * vec.y, group_delta_ * -v
ec.x);
        PointD pt2 = TranslatePoint(ptQ, group_delta_ * -vec.y, group_delta_ * v
ec.x);
        // get 2 vertices along one edge offset
        PointD pt3 = GetPerpendicD(path[k], norms[k], group_delta_);
        if (j == k)
        {
                PointD pt4 = PointD(pt3.x + vec.x * group_delta_, pt3.y + vec.y
* group_delta_);
                PointD pt = ptQ;
...
{
        if (deltaCallback64_) {
                // when deltaCallback64_ is assigned, group_delta_ won't be cons
tant,
                // so we'll need to do the following calculations for *every* ve
rtex.
                double abs_delta = std::abs(group_delta_);
                double arcTol = (arc_tolerance_ > floating_point_tolerance ?
                        std::min(abs_delta, arc_tolerance_) : abs_delta * arc_co
nst);
                double steps_per_360 = std::min(PI / std::acos(1 - arcTol / abs_
delta), abs_delta * PI);
        ...
        Point64 pt = path[j];
        PointD offsetVec = PointD(norms[k].x * group_delta_, norms[k].y * group_
delta_);
    ...
    path_out.emplace_back(pt.x + offsetVec.x, pt.y + offsetVec.y);
        ...
        {
                offsetVec = PointD(offsetVec.x * step_cos_ - step_sin_ * offsetV
ec.y,
...
void ClipperOffset::OffsetPoint(Group& group, const Path64& path, size_t j, size
_t k)
{
        ...
        if (cos_a > -0.999 && (sin_a * group_delta_ < 0)) // test for concavity
first (#593)
        {
                // is concave
                // by far the simplest way to construct concave joins, especiall
y those joining very
                // short segments, is to insert 3 points that produce negative r
egions. These regions
                // will be removed later by the finishing union operation. This
is also the best way
                // to ensure that path reversals (ie over-shrunk paths) are remo
ved.
#ifdef USINGZ
        path_out.emplace_back(GetPerpendic(path[j], norms[k], group_delta_), pat
h[j].z);
        path_out.emplace_back(path[j]); // (#405, #873, #916)
        path_out.emplace_back(GetPerpendic(path[j], norms[j], group_delta_), pat
h[j].z);
#else
        ...
        {
                // almost straight - less than 2.5 degree (#424, #482, #526 & #7
24)
...
{
        path_out.clear();
        for (Path64::size_type j = 0, k = path.size() - 1; j < path.size(); k =
j, ++j)
...
void ClipperOffset::DoGroupOffset(Group& group)
{
        ...
        if (group.join_type == JoinType::Round || group.end_type == EndType::Rou
nd)
        {
                // calculate the number of steps required to approximate a circl
e
                // (see https://www.angusj.com/clipper2/Docs/Trigonometry.htm)
                // arcTol - when arc_tolerance_ is undefined (0) then curve impr
ecision
                // will be relative to the size of the offset (delta). Obviously
 very
...
delta), abs_delta * PI);
        ...
        for ( ; path_in_it != group.paths_in.cend(); ++path_in_it)
        {
                ...
                if (pathLen == 1) // single point
                {
                        if (deltaCallback64_)
                        {
                                group_delta_ = deltaCallback64_(*path_in_it, nor
ms, 0, 0);
                                if (group.is_reversed) group_delta_ = -group_del
ta_;
                ...
                size_t steps = steps_per_rad_ > 0 ? static_cast<size_t>(std::cei
l(steps_per_rad_ * 2 * PI)) : 0; //#617
                        ...
                        else
                        {
                                ...
                                Rect64 r = Rect64(pt.x - d, pt.y - d, pt.x + d,
pt.y + d);
                                path_out = r.AsPath();
#ifdef USINGZ
                                for (auto& p : path_out) p.z = pt.z;
...
_it);
                else if (end_type_ == EndType::Joined) OffsetOpenJoined(group, *
path_in_it);
...
size_t ClipperOffset::CalcSolutionCapacity()
{
        ...
        for (const Group& g : groups_)
                result += (g.end_type == EndType::Joined) ? g.paths_in.size() *
2 : g.paths_in.size();
        return result;
}
...
void ClipperOffset::ExecuteInternal(double delta)
{
        ...
        if (std::abs(delta) < 0.5) // ie: offset is insignificant
        {
                ...
                for (const Group& group : groups_) sol_size += group.paths_in.si
ze();
                solution->reserve(sol_size);
                for (const Group& group : groups_)
                        copy(group.paths_in.begin(), group.paths_in.end(), back_
inserter(*solution));
        }
        else
        {
