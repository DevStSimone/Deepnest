...
//Bevel  : Similar to Square, but the offset distance varies with angle (simple
code & faster)
...
typedef std::function<double(const Path64& path, const PathD& path_normals, size
_t curr_idx, size_t prev_idx)> DeltaCallback64;
...
private:
    ...
    std::optional<size_t> lowest_path_idx{};
                ...
                JoinType join_type;
                EndType end_type;
                Group(const Paths64& _paths, JoinType _join_type, EndType _end_t
ype);
        };
...
; }
        ...
        void PreserveCollinear(bool preserve_collinear){preserve_collinear_ = pr
eserve_collinear;}

        bool ReverseSolution() const { return reverse_solution_; }
        void ReverseSolution(bool reverse_solution) {reverse_solution_ = reverse
_solution;}
